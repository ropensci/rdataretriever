...
It has been used in hundreds of research projects including research on biodiversity gradients [@hurlbert2003], ecological forecasting [@harris2019], and bird declines [@wilsey2019].

However working with the Breeding Bird Survey data can be challening because it is composed of roughly 100 different files that need to be cleaned and then combined in multiple ways.
...
The `rdataretriever` uses a Python package for much of the processing.
Fortunately Python, the Python package, and all of it's dependencies can be installed from inside R.
...
This creates an isolated Python installation that is just used from within R so that it doesn't conflict with any existing Python work.
It is also possible to work with an existing Python evironment if you prefer.
